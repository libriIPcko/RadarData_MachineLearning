...
for j in range (nu_params.shape[0]):
    ...
    while (i < lastFrame):
        ...
        model = OneClassSVM(kernel=kernel, coef0=coef, nu=nu_params[j]).fit(data)
        # Prediction
        ...
        plt.title("Frame: %d\n params kernel:%s, coef: %f ,nu: %f" %(i,kernel,coef,nu_params[j]))
        ...
        fig.savefig(f"SVM_an_vNUanalysis/{fileName}_pic_{j:04d}_{kernel}_coef {gamma}__nu {nu_params[j]}.png")
        # Load the image and write it to the video file
        img = cv2.imread(f"SVM_an_vNUanalysis/{fileName}_pic_{j:04d}_{kernel}_coef {gamma}__nu {nu_params[j]}.png")

...
for j in range (poly_params.shape[0]):
    ...
    while (i < lastFrame):
        ...
        fig.savefig(f"figure_radDat_SVM_AnomalyDet/{fileName}_pic_{j:04d}_{kernel}_gamma {actual_gamma}__nu {actual_nu}__poly {actual_poly}.png")
        # Load the image and write it to the video file
        img = cv2.imread(f"figure_radDat_SVM_AnomalyDet/{fileName}_pic_{j:04d}_{kernel}_gamma {gamma}__nu {actual_nu}__poly {actual_poly}.png")

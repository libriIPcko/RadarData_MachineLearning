...
bandwidth = np.linspace(0.4,1.6,num=40)
...
for j in range (bandwidth.shape[0]):
    # Prepare data
    #startFrame = i = 2
    ...
    while (i < lastFrame):
        ...
        fig = plt.gcf()
        plt.title("number of estimated clusters : %d \n in frame: %d  bandwidth: %f" % (n_clusters_, focusedFrame, bandwidth[j]))
        #fig.savefig(f"visualizedOutput_mean-shift_v1/m{measurement}/frame_{i:04d}_quantile_{quantile[j]}.png")
        fig.savefig(f"visualizedOutput_mean-shift_v1/{j}_count_frame_{i:04d}_bandwidth{bandwidth[j]}.png")
        ...
        print(f"Current progress: %d/%d quantile: {bandwidth[j]} - Duration: %.5f" % (focusedFrame, lastFrame, (endTime - startTime)))
        i = i + 1

...
vid_fileName = f"SVM_an_vGammaAnalysis/"+path.split('/')[-1].split('.')[-2]+".mp4"
...
for j in range (gamma_params.shape[0]):
    ...
    while (i < lastFrame):
        ...
        fig.savefig(f"SVM_an_vGammaAnalysis/{fileName}_pic_{j:04d}_{kernel}_gamma {actualGamma}__nu {actualNu}.png")
        # Load the image and write it to the video file
        img = cv2.imread(f"SVM_an_vGammaAnalysis/{fileName}_pic_{j:04d}_{kernel}_gamma {actualGamma}__nu {actualNu}.png")
